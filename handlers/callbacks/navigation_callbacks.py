...
class NavigationCallbackHandler:
    ...
    async def _handle_toggle_daily_digest(self, update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
        ...
        from handlers.common.settings import toggle_daily_digest

        state = callback_data.replace("toggle_daily_digest_", "")
        await toggle_daily_digest(update, context, state)
