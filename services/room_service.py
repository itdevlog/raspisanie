...
class RoomService(BaseScheduleService):
    ...
    def get_day(self, room_name: str, date: datetime) -> dict:
        ...
        room_id = self.find_room_id(room_name)
        if not room_id:
            raise EntityNotFoundError('room', room_name, f"❌ Кабинет '{room_name}' не найден")

        period_id = self._get_period_for_date(date)
        if not period_id:
            raise PeriodNotFoundError()

        payload = self._day_payload('room', self.school_data['ROOMS'][room_id], date)
