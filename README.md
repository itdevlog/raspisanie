...
- 🔄 **Автоматический мониторинг замен** — фоновый сервис проверяет данные (сегодня **и завтра**), `ExchangeDetector` ловит новые замены и шлёт уведомления.
- 💾 **Автосохранение** — выбранные школа и класс запоминаются в локальной JSON-БД.
- ⚙️ **Гибкие настройки** — смена школы/класса, вкл/выкл уведомлений.
...
- 🛠️ **Качество кода** — 96 юнит/интеграционных тестов, ruff (чистый), CI.
...
└── tests/                       # 96 pytest (юнит + интеграционные моки)
```
...
| `/settings` | Настройки уведомлений (+ админ-переключатель `update_notifications`) |
| `/week <класс>` | Расписание на неделю (напр. `/week 5и`) |
| `/school` | Информация о школе |
| `/admin` или `/stats` | Интерактивная админ-панель (только для `ADMIN_IDS`) |
...
.venv/bin/python -m pytest    # 96 тестов
```
