...
> ⏭️ Фаза 4 (новые возможности) — остаётся впереди; ориентир — §4 ниже. Сознательно отложены крупные сквозные рефакторинги: слой данных (`UserRepository`, `TypedDict`) и инъекция часов (clock).
...
Есть pytest (110 тестов: юнит + mock-интеграционные через pytest-asyncio, `tests/test_integration.py`). 
