...
- **Детектор смотрит только «сегодня»** — `background_updater.py:216`, `exchange_detector.py:126`: замены на завтра/неделю не обнаруживаются; после полуночи вчерашние исчезают. Дата в заголовке уведомления подменяется текущей (`notification_service.py:99`). → ✅ **исправлено 11.09**: `_check_exchange_updates` и `force_check_exchanges` проверяют замены на сегодня **и завтра**; дата замены (`date`) пробрасывается в `_format_exchange_for_notification` и в `timestamp`, поэтому заголовок уведомления показывает верный день, а не «сегодня».
...
- **Кэш расписания не инвалидируется после фонового обновления** — ключи без версии данных, пользователи до 10 минут видят старое (`schedule_service.py:20-93`, TTL в `bot.py:62`). Добавить в ключ хэш данных + явный `cache_service.clear()` в `_perform_update`. → ✅ **исправлено 11.09**: `_perform_update` вызывает `cache_service.clear()` после обновления данных (и при принудительном обновлении из админки — тот же путь).
...
> **11.09.2026**: добавлен pytest (`pytest.ini`), чистые юнит-тесты: разбивка длинных сообщений и её безпотерьность, `paginate`, `get_display_name`, матчинг цифры класса (p.11). `requirements.txt` очищен от мёртвых комментариев. Ниже — приоритетные (теперь частично покрыты).
