...
- **`ExchangeService` создаётся в цикле** — `exchange_detector.py:102`: на каждую школу каждый тик. → ✅ **исправлено 11.09**: убран лишний объект `ExchangeService` из `_get_current_exchanges` (не использовался) и неиспользуемый импорт.
...
- «Список устарел» оставляет мёртвую клавиатуру. → ✅ **исправлено 11.09**: при устаревшем списке (поиск/полный) перерисовывается актуальный список (teacher/room callbacks) вместо тоста.
...
- Ввод поиска без ограничения длины и кнопки «Отмена». → ✅ **исправлено 11.09**: кнопка «❌ Отмена» (снимает флаг и возвращает в меню) в `EntityMenuHandler::search_input`; ограничение длины запроса до 80 символов в `class_schedule.py`.
...
- **Кэш уведомлений** — «последние 100» через `list(set)[-100:]`, порядок не гарантирован; очистки по 24 ч не было. → ✅ **исправлено 11.09**: `sent_notifications` хранит `{key: timestamp}`; `_cleanup_old_notifications` удаляет записи старше 24 ч (вместо случайного среза), старый set-формат совместим на чтение.
- **Дублирование запросов в `data_loader`** — вложенные ретраи дают до 9 запросов на школу; нет ETag/If-Modified-Since; `except Exception` ловит и `JSONDecodeError` (ретрай бессмысленен); парсинг по `'var NIKA='` хрупок; `Session` не закрывается. → 🟡 **частично 11.09**: `JSONDecodeError` больше не ретраится (выходим сразу), добавлен `DataLoader.close()`.
- **`CacheService`** — нет лимита размера и потокобезопасности; `get_stats` сериализует весь кэш в строки. → ✅ **исправлено 11.09**: `RLock`, лимит `max_entries` + вытеснение самых старых, `get_stats` без полной сериализации.
...
- Статус старше 24 ч должен показывать 🔴 «Устарело» (`status_service.py:68`), в админке выводится ⚠️ — привести к единому виду. → ✅ **исправлено 11.09**: единый хелпер `status_service::status_icon` (берёт иконку из `status['status']`), применён в админ-панели и `/status` — 🔴 «Устарело» теперь видно и в админке.
