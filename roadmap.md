...
> ✅ 11.09 (Фаза 1 — критические исправления, P0): кэш расписания привязан к `school_id` (Task 4); кэш замен привязан к дате + атомарная запись (Task 5); блокирующий детект замен вынесен в `asyncio.to_thread` с единым flush (Task 6); `KeyError` при неполных данных (Task 2); `update.message` → `effective_message` (Task 3); пагинация сохраняет тип расписания (Task 7); валидация несуществующей школы + сброс флагов поиска (Task 8); общий `GENERIC_ERROR_MSG` в глобальном обработчике (Task 9). Подробности — в [CHANGELOG.md](CHANGELOG.md). Открытых P0/P1 нет.
