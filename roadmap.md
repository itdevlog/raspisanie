...
- **`FileDB` перезаписывает весь JSON на каждую операцию** — долговечность закрыта 16.09 (`fsync` + `.bak`); остаётся *отложенная запись* (dirty-флаг) или переход на `sqlite3` (stdlib). (Рискованно менять на живой системе — делать отдельно.)
